...
def get_political_news(region: str = ""):
    """
    Fetch high-impact political news using NewsAPI and GNews. Optionally filter by region (country name or code).
    Returns a list of news dicts with 'title', 'description', and 'url'.
    """
    api_key = os.getenv('NEWSAPI_KEY')
    gnews_api_key = os.getenv('GNEWS_API_KEY')
    # Map common country/region names to NewsAPI 2-letter codes
    country_map = {
    ...
    if api_key:
        ...
        params = {'q': 'politics OR election OR government OR parliament OR bill OR law', 'language': 'en', 'page_size': 10}
        if country_code:
    ...
    if gnews_api_key and not newsapi_results:
        gnews_url = f'https://gnews.io/api/v4/top-headlines?token={gnews_api_key}&lang=en&max=10&q=politics'
        if country_code:
    ...
    if not newsapi_results and not gnews_results:
        query = region_key + ' politics' if region_key else 'politics'
        encoded_query = urllib.parse.quote(query)
        rss_url = f'https://news.google.com/rss/search?q={encoded_query}&hl=en-IN&gl=IN&ceid=IN:en' if region_key else 'https://news.google.com/rss/search?q=politics&hl=en-IN&gl=IN&ceid=IN:en'
    ...
    if not all_results:
        ...
        if api_key:
            everything_params = {'q': 'politics', 'language': 'en', 'page_size': 10}
            try:
        ...
        if gnews_api_key and not all_results:
            gnews_url = f'https://gnews.io/api/v4/search?token={gnews_api_key}&lang=en&max=10&q=politics'
            if country_code:
                gnews_url += f'&country={country_code}'
            resp = requests.get(gnews_url, timeout=10)
            if resp.status_code == 200:
    ...
    if not all_results:
        return [{
            ...
            'description': 'No trending political news found for your region or globally at this time.',
            'url': ''
        }]
    return all_results 