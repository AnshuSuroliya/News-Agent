...
def get_general_news(region: str = ""):
    """
    Fetch the most important and trending general news stories using NewsAPI and GNews. Optionally filter by region (country name or code).
    ...
    if api_key:
        ...
        params = {'language': 'en', 'page_size': 10}
        if country_code:
    ...
    if gnews_api_key and not newsapi_results:
        gnews_url = f'https://gnews.io/api/v4/top-headlines?token={gnews_api_key}&lang=en&max=10'
        if country_code:
    ...
    if not newsapi_results and not gnews_results:
        query = region_key if region_key else ''
        encoded_query = urllib.parse.quote(query)
        rss_url = f'https://news.google.com/rss/search?q={encoded_query}&hl=en-IN&gl=IN&ceid=IN:en' if region_key else 'https://news.google.com/rss?hl=en-IN&gl=IN&ceid=IN:en'
    ...
    if not all_results:
        ...
        if api_key:
            everything_params = {'q': '', 'language': 'en', 'page_size': 10}
            try:
        ...
        if gnews_api_key and not all_results:
            gnews_url = f'https://gnews.io/api/v4/search?token={gnews_api_key}&lang=en&max=10&q='
            if country_code:
                gnews_url += f'&country={country_code}'
            resp = requests.get(gnews_url, timeout=10)
            if resp.status_code == 200:
    ...
    if not all_results:
        return [{
            ...
            'description': 'No trending news found for your region or globally at this time.',
            'url': ''
        }]
    return all_results 